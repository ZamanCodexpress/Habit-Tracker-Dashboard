...
const Insights = {
  ...
  generateTips() {
    ...
    if (bestDay !== worstDay) {
      tips.push(
        `💡 You tend to miss habits on **${worstDay}s**. Try setting a reminder?`,
      );
    }
    ...
    if (struggles.length > 0) {
      ...
      tips.push(
        `📉 **${name}** seems tough lately. Maybe try reducing the frequency or target?`,
      );
    }
    ...
    if (stars.length > 0) {
      tips.push(
        `⭐ You're crushing **${stars[0].name}**! Have you considered increasing the difficulty?`,
      );
    }
